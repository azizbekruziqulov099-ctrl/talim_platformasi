"""Production runtime for SamTM V19.
...
CACHEABLE_PREFIXES = tuple(x.strip() for x in os.getenv(
    "MICROCACHE_PATHS",
    "/api/maktab/dashboard,/api/maktab/yuklama_xulosasi,/api/maktab/aqlli_holatlar,/api/maktab/aqlli_jadval/v2/sozlamalar,/api/versiya"
).split(",") if x.strip())
...
HEAVY_PATH_PARTS = ("/yaratish", "/xodim_import", "/shablon_import", "/import")
...
def register_runtime(app, platform, school):
    ...
    def live(): return {"status":"ok","version":"v19.0"}
    ...
    def ready():
        ...
        try:
            ...
            return {"status":"ready","db":bool(cur.fetchone()["ok"]),"schema_ready":len(_SCHEMA_READY)}
        finally:
    ...
    def perf(token: str):
        ...
        return {"version":"19.0","max_inflight_per_worker":MAX_INFLIGHT,"max_heavy_per_worker":MAX_HEAVY,"db_pool_per_worker":platform._DB_POOL_MAX,"microcache_ttl":CACHE_TTL,"redis":bool(_REDIS),"schema_helpers_cached":sorted(_SCHEMA_READY)}
