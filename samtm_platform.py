...
def _v1877_group_template_catalog(cur, maktab_id: int):
    ...
    type_names = {
        "alphabet": "Alifbo bo‘yicha guruhlar",
        "gender": "O‘g‘il / Qiz",
...
async def xodim_import(token: str, maktab_id: int, fayl: UploadFile = File(...)):
    ...
    guruh_nomlari = {
        ...
        "3guruh": ("group_3", "3-guruh"),
        "4guruh": ("group_4", "4-guruh"),
    }
    ...
    def guruh_moslashtir(sinf_nomi, value):
        ...
        if kalit in guruh_nomlari:
            ...
            if guruh_kaliti in ("group_1", "group_2", "group_3", "group_4") and "alphabet" not in turlar and "manual" not in turlar:
                raise ValueError(f"{sinf_nomi} avval Alifbo bo'yicha guruhlarga ajratilsin")
            return guruh_kaliti, guruh_nomi
        raise ValueError("guruhni Butun sinf, O'g'il bolalar, Qiz bolalar yoki 1–4-guruhdan tanlang")
...
def maktab_sinflari_royxati(token: str, maktab_id: int):
    ...
    _maktab_sinflari_jadvali(cur)
    cur.execute("""
        SELECT s.id, s.sinf, s.harf, s.smena, s.bino, s.xona,s.bino_id,s.xona_id,s.guruhlash_usuli,s.guruh_soni,
...
def maktab_sinf_yarat(sorov: SinfYaratish):
    ...
    guruh_soni = _sinf_guruh_soni_normalizatsiya(guruhlash, sorov.guruh_soni)
    paroli = "".join(secrets.choice(string.digits) for _ in range(4))
    cur.execute("""
        INSERT INTO maktab_sinflari(maktab_id,sinf,harf,smena,rahbar_user_id,qoshilish_paroli,guruhlash_usuli,guruh_soni)
        VALUES(%s,%s,%s,%s,%s,%s,%s,%s)
        ON CONFLICT (maktab_id, sinf, harf) DO UPDATE SET
            ...
            smena=EXCLUDED.smena,guruhlash_usuli=EXCLUDED.guruhlash_usuli,guruh_soni=EXCLUDED.guruh_soni
        RETURNING id, qoshilish_paroli
    """, (sorov.maktab_id,sinf,harf,sorov.smena,sorov.rahbar_user_id,paroli,guruhlash,guruh_soni))
    natija = cur.fetchone()
    conn.commit()
    cur.close()
    conn.close()
...
def _sinf_guruh_soni_normalizatsiya(usul, guruh_soni):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Guruh soni raqam bo'lishi kerak") from error
    if soni not in (2, 3, 4):
        raise HTTPException(status_code=400, detail="Guruh soni 2, 3 yoki 4 bo'lishi kerak")
    return soni


def _sinf_guruh_sonlarini_ol(cur, sinf_id, guruh_soni):
    cur.execute(
        """SELECT guruh_raqami,COUNT(*) AS soni
           FROM maktab_sinf_azolari
           WHERE sinf_id=%s AND guruh_raqami BETWEEN 1 AND %s
           GROUP BY guruh_raqami""",
        (sinf_id, guruh_soni),
    )
    topilgan = {int(row["guruh_raqami"]): int(row["soni"] or 0) for row in cur.fetchall()}
    return {str(raqam): topilgan.get(raqam, 0) for raqam in range(1, guruh_soni + 1)}


def _sinf_guruhlarini_qayta_taqsimla(cur, sinf_id):
    """Sinf sozlamasiga ko'ra o'quvchilarni barqaror 2/3/4 guruhga ajratadi."""
    ensure_school_wizard_columns(cur)
    _sinf_azolari_jadvali(cur)
    cur.execute("SELECT guruhlash_usuli,guruh_soni FROM maktab_sinflari WHERE id=%s", (sinf_id,))
    sinf = cur.fetchone()
    if not sinf:
        raise HTTPException(status_code=404, detail="Sinf topilmadi")
    usul = (sinf["guruhlash_usuli"] or "none").strip().lower()
    guruh_soni = _sinf_guruh_soni_normalizatsiya(usul, sinf["guruh_soni"])
    if usul == "manual":
        return _sinf_guruh_sonlarini_ol(cur, sinf_id, guruh_soni)
    if usul == "none":
        cur.execute(
    ...
    """, (sinf_id,))
    azolar = cur.fetchall()
    guruhlar = {raqam: [] for raqam in range(1, guruh_soni + 1)}
    if usul == "alphabet":
        asosiy, ortiqcha = divmod(len(azolar), guruh_soni)
        boshlanish = 0
        for guruh_raqami in range(1, guruh_soni + 1):
            hajm = asosiy + (1 if guruh_raqami <= ortiqcha else 0)
            guruhlar[guruh_raqami] = azolar[boshlanish:boshlanish + hajm]
            boshlanish += hajm
    else:
        nomalum = []
        for azo in azolar:
    ...
    return {str(raqam): len(qatorlar) for raqam, qatorlar in guruhlar.items()}
...
def _sinf_guruh_tizimini_taqsimla(cur, tizim_id):
    ...
    if tizim["turi"] == "alphabet":
        guruh_soni = _sinf_guruh_soni_normalizatsiya("alphabet", tizim["guruh_soni"])
        bolimlar = {str(raqam): [] for raqam in range(1, guruh_soni + 1)}
        asosiy, ortiqcha = divmod(len(azolar), guruh_soni)
        boshlanish = 0
        for raqam in range(1, guruh_soni + 1):
            hajm = asosiy + (1 if raqam <= ortiqcha else 0)
            bolimlar[str(raqam)] = azolar[boshlanish:boshlanish + hajm]
            boshlanish += hajm
        kalitlar = {str(raqam): (f"group_{raqam}", f"{raqam}-guruh") for raqam in range(1, guruh_soni + 1)}
    else:
        bolimlar = {"1": [], "2": []}
        nomalum = []
        for azo in azolar:
...
def maktab_sinf_guruh_tizimini_sozla(sorov: SinfGuruhTizimiSozlash):
    ...
    cur.execute("SELECT maktab_id,guruh_soni FROM maktab_sinflari WHERE id=%s", (sorov.sinf_id,))
    sinf = cur.fetchone()
    if not sinf:
        cur.close(); conn.close()
        raise HTTPException(status_code=404, detail="Sinf topilmadi")
    ...
    if sorov.faol and turi in ("gender", "alphabet"):
        joriy_soni = int(sinf.get("guruh_soni") or 2)
        kerakli_soni = 2 if turi == "gender" else (joriy_soni if joriy_soni in (2, 3, 4) else 2)
        if kerakli_soni != joriy_soni:
            cur.execute("UPDATE maktab_sinflari SET guruh_soni=%s WHERE id=%s", (kerakli_soni, sorov.sinf_id))
            sinf["guruh_soni"] = kerakli_soni
    if not sorov.faol:
        cur.execute(
    ...
    mavjud_guruh_soni = int(sinf.get("guruh_soni") or 2)
    if legacy_usul == "none":
        legacy_guruh_soni = 1
    elif legacy_usul == "gender":
        legacy_guruh_soni = 2
    else:
        legacy_guruh_soni = mavjud_guruh_soni if mavjud_guruh_soni in (2, 3, 4) else 2
    cur.execute(
        "UPDATE maktab_sinflari SET guruhlash_usuli=%s,guruh_soni=%s WHERE id=%s",
        (legacy_usul, legacy_guruh_soni, sorov.sinf_id),
    )
...
def maktab_sinflar_katalogi(token: str, maktab_id: int):
    ...
    cur.execute("""
        SELECT s.id,s.sinf,s.harf,s.smena,s.bino,s.xona,s.guruhlash_usuli,s.guruh_soni,
...
def maktab_sinf_azolarini_guruhla(sorov: SinfAzolariniGuruhlash):
    ...
    elif re.fullmatch(r"group_[1-4]", amal):
        guruh_raqami = int(amal.rsplit("_", 1)[1])
        ruxsat_soni = _sinf_guruh_soni_normalizatsiya("manual", sinf["guruh_soni"])
        if guruh_raqami > ruxsat_soni:
            cur.close(); conn.close()
            raise HTTPException(status_code=400, detail=f"Bu sinfda faqat {ruxsat_soni} ta guruh belgilangan")
        cur.execute(
    ...
    guruh_sonlari = _sinf_guruh_sonlarini_ol(
        cur,
        sorov.sinf_id,
        _sinf_guruh_soni_normalizatsiya("manual", sinf["guruh_soni"]),
    )
    conn.commit(); cur.close(); conn.close()
    return {
        "holat": "saqlandi",
        "yangilangan": len(user_ids),
        "guruhlar": guruh_sonlari,
    }
...
def maktab_sinf_sozlash(sorov: MaktabSinfSozlash):
    ...
    conn = _db(); cur = conn.cursor()
    _maktab_sinflari_jadvali(cur); _sinf_azolari_jadvali(cur)
    cur.execute("SELECT maktab_id,smena,bino_id,xona_id,guruhlash_usuli,guruh_soni FROM maktab_sinflari WHERE id=%s", (sorov.sinf_id,))
    sinf = cur.fetchone()
    if not sinf:
        cur.close(); conn.close()
        raise HTTPException(status_code=404, detail="Sinf topilmadi")
    ...
    if guruhlash not in ("none", "gender", "alphabet", "manual"):
        cur.close(); conn.close()
        raise HTTPException(status_code=400, detail="Guruhlash usuli noto'g'ri")
    guruh_soni = _sinf_guruh_soni_normalizatsiya(
        guruhlash,
        sorov.guruh_soni if sorov.guruh_soni is not None else sinf["guruh_soni"],
    )
    ...
    if xona_id is not None:
        ...
        bino_nomi = bino["nomi"]
    cur.execute("""
        UPDATE maktab_sinflari SET smena=%s,bino_id=%s,xona_id=%s,bino=%s,xona=%s,guruhlash_usuli=%s,guruh_soni=%s
        WHERE id=%s
    """, (smena,bino_id,xona_id,bino_nomi,xona_raqami,guruhlash,guruh_soni,sorov.sinf_id))
