...
def topik_fan_nomini_tahrirla(token: str, sinf: str, eski_fan: str, yangi_fan: str):
    ...
    _admin_tekshir(token)
    eski = eski_fan.strip()
    yangi = yangi_fan.strip()
    if not eski or not yangi:
        raise HTTPException(status_code=400, detail="Eski va yangi fan nomini kiriting")
    if eski.casefold() == yangi.casefold():
    ...
    try:
        ...
        conn.rollback()
        raise HTTPException(status_code=500, detail=f"Fan nomini saqlab bo'lmadi: {exc}")
    finally:
        cur.close()
        conn.close()
    return {"holat": "saqlandi", "yangilangan_soni": yangilangan, "fan": yangi}
...
def topik_mavzu_nomini_tahrirla(token: str, topic_codes: str, yangi_mavzu: str):
    ...
    try:
        ...
        yangilangan = cur.rowcount
        if not yangilangan:
            raise HTTPException(status_code=404, detail="Tahrirlanadigan mavzu topilmadi")
        conn.commit()
    except HTTPException:
        conn.rollback()
        raise
        ...
        conn.rollback()
        raise HTTPException(status_code=500, detail=f"Mavzu nomini saqlab bo'lmadi: {exc}")
    finally:
        cur.close()
        conn.close()
    return {"holat": "saqlandi", "yangilangan_soni": yangilangan, "mavzu": yangi}
