...
def _v1852_build_jobs(classes, loads, assignments, group_settings, teachers):
    ...
    for assignment in assignments:
        ...
        whole_quotas[key].extend(
            [int(teacher_id)] * max(0, int(assignment.get("haftalik_soat") or 0))
        )
...
def _v195_refresh_teacher_summary(cur, maktab_id: int, user_id: int):
    ...
    weekly_total = sum(int(row.get("haftalik_soat") or 0) for row in rows)
...
def _v195_reconcile_teacher_loads_with_plan(cur, maktab_id: int):
    ...
    for row in assignments:
        ...
        old_hours = max(0, int(row.get("haftalik_soat") or 0))
        new_hours = min(old_hours, max(0, int(remaining[exact])))
        if new_hours <= 0:
            cur.execute("DELETE FROM maktab_dars_birikmalari WHERE id=%s", (row["id"],))
            deleted += 1
            continue
...
class V192TeacherLoadSave(BaseModel):
    ...
    haftalik_maqsad_soat: Optional[int] = None
...
def _v194_teacher_profile_values(mutaxassisligi, haftalik_maqsad_soat):
    ...
    if target is not None:
        ...
        if target < 1 or target > 60:
            raise HTTPException(status_code=400, detail="Haftalik maqsad soati 1–60 oralig'ida bo'lishi kerak")
